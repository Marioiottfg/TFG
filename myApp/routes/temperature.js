...
router.get('/', function(req, res, next) {
	...
	var db = req.db;
	var tempCollection = db.get('mgiotTemperatura');
	tempCollection.find( {"time": { $gte: (new Date((new Date()).getTime() - (7 * 24 * 60 * 60 * 							1000)))}}, {sort: {"time": 1} }, function(e, data){
		// Se cargan los datos en la pagina y se renderiza
		if(data != null) res.render('temperature', {"temperature": JSON.stringify(data)});
	});
